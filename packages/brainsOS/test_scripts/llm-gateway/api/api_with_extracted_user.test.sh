...
# Validate USER_SUB immediately after extraction
if [ -z "$USER_SUB" ] || [ "$USER_SUB" == "null" ]; then
    echo -e "${RED}Error: Could not extract valid user ID (sub) from token${NC}"
    exit 1
fi
...
call_api() {
    ...
    # Ensure USER_SUB is valid for test verification purposes
    if [ -z "$USER_SUB" ] || [ "$USER_SUB" == "null" ]; then
        echo -e "${RED}Error: No valid user ID available from token${NC}"
        exit 1
    fi
    ...
    # No longer adding userId to payload - it will be extracted from the token
    # The backend will ignore any userId in the payload to prevent impersonation
    
    echo -e "${BLUE}Payload: ${payload}${NC}"
    echo -e "${BLUE}Token contains userId: ${USER_SUB}${NC}"
...
call_api "list-conversations" '{}' "LIST CONVERSATIONS: Get all conversations for the user"
